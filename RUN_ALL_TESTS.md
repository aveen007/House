...
## Быстрый запуск всех тестов (кроме JMeter)

Создайте скрипт для последовательного запуска:
...
echo ""
echo "=== 2. Volume Testing ==="
mvn -Dtest=VolumeTestingTest test || exit 1

echo ""
echo "=== 3. Business Cycle Testing ==="
mvn -Dtest=BusinessCycleTest test || exit 1

echo ""
echo "=== 4. API Functional Smoke Testing ==="
./scripts/api-functional-smoke.sh || exit 1

echo ""
echo "=== 5. Database Integrity Testing ==="
./scripts/run-db-integrity-checks.sh || exit 1

echo ""
echo "=== 6. Security & Access Control Testing ==="
./scripts/security-access-tests.sh || exit 1

echo ""
echo "=== 7. Performance Profiling ==="
./scripts/performance-profiling.sh || exit 1

echo ""
echo "=== Все тесты завершены успешно! ==="
```

**Сохраните как** `House/run_all_tests.sh` и сделайте исполняемым:
```bash
chmod +x House/run_all_tests.sh
```
...
1. **Unit Tests** (быстро, проверяет базовую функциональность)
2. **Business Cycle Testing** (проверяет полный цикл)
3. **Volume Testing** (проверяет производительность с большими данными)
4. **Database Integrity Testing** (проверяет целостность БД)
5. **API Functional Smoke Testing** (проверяет API через HTTP)
6. **Security & Access Control Testing** (проверяет безопасность)
7. **Performance Profiling** (измеряет производительность)
8. **Load Testing** (нагрузочное тестирование - долго)
9. **Stress Testing** (стресс-тестирование - очень долго)

---

## Примечания

- **JMeter тесты** (Load/Stress) требуют много времени и ресурсов
- Запускайте их отдельно, когда есть время
- Убедитесь, что сервер и БД запущены перед запуском API тестов
- Результаты сохраняются в соответствующих директориях

---

## Проверка результатов

- **Maven тесты**: `target/surefire-reports/`
- **API/Security/Performance скрипты**: вывод в консоль
- **JMeter тесты**: `jmeter-tests/results/`
